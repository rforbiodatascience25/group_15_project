...
p<-ggplot(kidney_filter, aes(age_group, fill = classification)) +
  geom_bar(position = position_dodge(preserve = "single")) +
  scale_fill_manual(values = c("#FFB86C", "#8BE9FD")) +
  theme_minimal() +
  labs(title="Kidney disease distribution",
       x = ("Age Group"),
       y = ("Count"),
       fill = "Classification"
       )

ggsave("../plots/04_ckd.png", plot = p, width = 8, height = 4)
...
As expected, we see that most of the individuals not diagnosed with CKD fall within the Normal to Mild category. On the contrary, among individuals with CKD, the majority of individuals belong to advanced stages of the disease, with GFR from 30 to 40, and over 60 people suffer from kidney failure.
...
p_ckd_stage<-ggplot(kidney_heatmap, aes(x = cdk_stage, y = classification, fill = count)) +
