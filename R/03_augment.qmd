...
-   summary statistics to begin the analysis
...
calculate_GFR <- function(dataframe) {
   dataframe |> 
    mutate( #pmax/pmin instead of max/min in order to iterate over each creatinine value
      GFR_male = 142 * pmin(serum_creatinine / 0.9, 1)**(-0.302) 
      * pmax(serum_creatinine / 0.9, 1) ** (-1.2) * (0.9938**age)
      ) |> 
    mutate(
      GFR_female = 142 * pmin(serum_creatinine / 0.7, 1)**(-0.241) 
      * pmax(serum_creatinine / 0.7, 1) ** (-1.2) * (0.9938**age) * 1.012
      ) |> 
     mutate(
       GFR_average = (GFR_female + GFR_male) / 2
       )
}
...
```{r}
write_csv(kidney_merged_GFR,
          "../data/03_data_augment.csv")
```
