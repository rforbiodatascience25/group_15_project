...
kidney_analysis |>
  write_rds("../data/04_data_analysis.rds")
```
...
ggsave(
    filename = "../results/06_gfr_by_ckd.png"
  )
...
ggsave(
    filename = "../results/06_CKD_dist.png"
  )
...
ggsave(
    filename = "../results/06_CKD_hypertension.png"
  )
```
...
ggsave(
    filename = "../results/06_CKD_diabetes.png"
  )

```

## Investigating hypertension and diabetes mellitus

Here we investigate why the two variables seem equally discriminatory but one is labelled as non important by the random forest analysis.
...
It looks like the variables are strongly correlated and that could explain what is happening.
...
The levels of key predictors - albumin, hemoglobin, cell volume, cell count of red blood, and creatinine are chosen for the final visualisation. RBC count and related packed cell volume are showing lower levels in individuals who are diagnosed with CKD. This is due to the kidneys not producing enough of hormone erythropoietin, responsible for signalling the creation of red blood cells in the bone marrow. This shortage can lead to anemia. This strongly correlates with hemoglobin levels and the expected result is shown in the plot - lower hemoglobin levels for diagnosed individuals. Creatinine levels, also used for GFR calculation, follow the opposite pattern. Creatinine, as a waste product from muscle and protein breakdown, is filtered by the kidneys. When their function is compromised, this product is found in the blood. As seen in the plot, healthy individuals show levels close to zero while elevated levels are recorded in diagnosed individuals. Lastly, albumin found in urine is an early indicator of kidney damage, as this essential protein should only stay in the blood. This is a clear indicator of a kidney filtration issue that can usually predict the progression of the disease. Non-diagnosed individuals show all scores 0, while diagnosed individuals range from 0 to 3.
...
ggsave(
    filename = "../results/06_key_predictors.png"
  )
